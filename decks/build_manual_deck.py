...
Usage:
    python decks/build_manual_deck.py --draft
    python decks/build_manual_deck.py --output decks/out/woodlands-manual-workshop.pptx
"""
    ...
    for relative in ("docs/16-manual-upload-labs.md", "docs/12-spark-environment.md"):
        ...
        for section in re.split(r"(?m)(?=^#{2,3} )", text):
            if filename in section:
                return f"Source: {relative}\nCapture: images/training/manual/{filename}\n\n{section.strip()}"
...
def focused_picture(capture: dict) -> tuple[io.BytesIO, tuple[int, int]]:
    ...
    path = IMAGES / capture["file"]
    raw = IMAGES / "raw" / capture["file"]
...
def screenshot_slide(presentation, capture: dict, number: int, total: int) -> None:
    ...
    text_box(slide, 0.55, 0.25, 10.8, 0.25, capture_section(source_number).upper(),
             10, INK, bold=True)
...
def build(output: Path, *, draft: bool = False) -> int:
    ...
    captures = json.loads((IMAGES / "annotations.json").read_text(encoding="utf-8"))["captures"]
    if not draft:
    ...
    for index, (heading, detail) in enumerate([
        ...
        ("Process", "Complete TODO exercises and verify each lab checkpoint."),
    ...
    label = "DRAFT: live rehearsal is not complete" if draft else "Verified portal walkthrough | September 2026"
...
def main() -> int:
    ...
    output = arguments.output or ROOT / "decks/out" / (
        "woodlands-manual-workshop-draft.pptx" if arguments.draft else "woodlands-manual-workshop.pptx")
    try:
