...
def test_given_download_handout_when_used_then_all_original_files_download(tmp_path):
    ...
    with sync_playwright() as playwright:
        ...
        image_root = ROOT / "scripts/verification/screenshots/manual-download/raw"
        page.evaluate("window.scrollTo(0,0)")
        page.screenshot(path=str(image_root / "18-manual-03-download.png"), full_page=True)
        for width, height in ((1280, 900), (390, 844)):
            ...
            page.screenshot(path=str(ROOT / ".copilot-tracking/manual-download" / f"download-{width}.png"), full_page=True)
        ...
        assert errors == []
        browser.close()
    (ROOT / ".copilot-tracking/manual-download/browser-results.json").write_text(json.dumps(report, indent=2) + "\n")
